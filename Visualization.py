...
def scat_plot(a,b,s):
    ...
    ax1.scatter(x, sorted(a), s=10, c='b', marker="s", label='Trump')
    ax1.scatter(y, sorted(b), s=10, c='r', marker="o", label='Hillary')

    if(s == 'pos'):
        ax1.scatter(y, [0.11585 for i in range(len(b))], s=10, c='black', marker="o", label='Mean Hillary')
        ...
        plt.show()
    if(s=='neg'):
        ax1.scatter(y, [0.10901 for i in range(len(b))], s=10, c='black', marker="o", label='Mean Hillary')
...
def main():
    ...
    hillary = open_file('sentiments_hillary.pickle')

    trumppos = create_list(trump, 'pos')
    hillarypos = create_list(hillary, 'pos')

    scat_plot(trumppos, hillarypos, 'pos')

    trumpneg = create_list(trump, 'neg')
    hillaryneg = create_list(hillary, 'neg')

    scat_plot(trumpneg, hillaryneg, 'neg')
